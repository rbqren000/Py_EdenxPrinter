...
- (void)setBlockOnScanBlock:(void(^)(CBCentralManager *centralManager))block {
    [[babySpeaker callback]setBlockOnScan:block];
}
...
-(void)setBlockOnStartConnectToPeripheral:(void(^)(CBPeripheral *peripheral))block{
    
    [[babySpeaker callback]setBlockOnStartConnectToPeripheral:block];
}
...
- (BOOL)isScanning{
    ...
    return [babyCentralManager isScanning];
}
...
- (void)start:(CBPeripheral *)cachedPeripheral {
    ...
    if (CENTRAL_MANAGER_INIT_WAIT_TIMES >= KBABY_CENTRAL_MANAGER_INIT_WAIT_TIMES ) {
        RBQLog4(@">>> 第%d次等待CBCentralManager 打开任然失败，请检查你蓝牙使用权限或检查设备问题。",CENTRAL_MANAGER_INIT_WAIT_TIMES);
        return;
    ...
    RBQLog4(@">>> 第%d次等待CBCentralManager打开",CENTRAL_MANAGER_INIT_WAIT_TIMES);
}
...
- (CBPeripheral *)retrievePeripheralWithUUIDString:(NSString *)UUIDString {
    ...
    @try {
        ...
        RBQLog4(@">>> retrievePeripheralWithUUIDString error:%@",exception)
    } @finally {
    }
    return p;
}
...
-(BOOL)isAdvertising{
    ...
    return babyPeripheralManager.peripheralManager.isAdvertising;
}
...
//停止发送广播
- (void)peripheralModelBlockOnDidStopdvertising:(void (^)(CBPeripheralManager *))block{
    
    [[babySpeaker callback] setBlockOnPeripheralModelDidStopAdvertising:block];
}

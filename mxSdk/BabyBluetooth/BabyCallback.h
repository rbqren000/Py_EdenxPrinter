...
//这里我自己添加一个开始扫描的black  作者 rbq
typedef void (^BBStartScanBlock)(CBCentralManager *centralManager);

//babyBluettooth cancelScanBlock方法调用后的回调
typedef void (^BBOnCancelScanBlock)(CBCentralManager *centralManager);

//  rbq 添加  开始连接
typedef void (^BBOnStartConnectToPeripheral)(CBPeripheral *peripheral);

// rbq 添加 断开连接
//typedef void (^BBCancelPeripheralConnectionBlock)(CBPeripheral *peripheral);

//babyBluettooth cancelAllPeripheralsConnection 方法调用后的回调，既所有的连接都断开后调用
...
//自己添加的停止发送广播的事件  ,暂时不计划使用，因为如果蓝牙直接被用户关掉的话，广播也就自动停止了，没做对应的处理，所以暂时不使用该事件
typedef void (^BBPeripheralModelDidStopAdvertising)(CBPeripheralManager *peripheral);
...
//这个black自己添加的
@property(nonatomic,copy) BBStartScanBlock blockOnScan;
//babyBluettooth stopScan方法调用后的回调
@property(nonatomic,copy) BBOnCancelScanBlock blockOnCancelScan;

//
@property(nonatomic,copy) BBOnStartConnectToPeripheral blockOnStartConnectToPeripheral;
...
@property (nonatomic, copy) BBPeripheralModelDidStopAdvertising blockOnPeripheralModelDidStopAdvertising;

/*
 BabyBluetooth
 简单易用的蓝牙ble库，基于CoreBluetooth 作者：任宝强
 https://github.com/coolnameismy/BabyBluetooth
 */

//  Created by 任宝强 on 15/7/30.
...
- (void)scanPeripherals {
    ...
    if (![centralManager isScanning]) {
        ...
        //这里是我自己添加的blockOnScan 作者 rbq
        if ([currChannel blockOnScan]) {
            [currChannel blockOnScan](centralManager);
        }
...
- (void)connectToPeripheral:(CBPeripheral *)peripheral{
    /*
    //自己改的，开始连接设备，则停止扫描  rbq
    [self cancelScan];
     */
    RBQLog4(@"***开始连接设备***");
    if ([currChannel blockOnStartConnectToPeripheral]) {
        [currChannel blockOnStartConnectToPeripheral](peripheral);
    }
...
- (void)centralManagerDidUpdateState:(CBCentralManager *)central {
    ...
    switch (central.state) {
        case CBManagerStateUnknown:
            RBQLog4(@">>>CBCentralManagerStateUnknown");
            break;
        case CBManagerStateResetting:
            RBQLog4(@">>>CBCentralManagerStateResetting");
            break;
        case CBManagerStateUnsupported:
            RBQLog4(@">>>CBCentralManagerStateUnsupported");
            break;
        case CBManagerStateUnauthorized:
            RBQLog4(@">>>CBCentralManagerStateUnauthorized");
            break;
        case CBManagerStatePoweredOff:
            RBQLog4(@">>>CBCentralManagerStatePoweredOff");
            break;
        case CBManagerStatePoweredOn:
            RBQLog4(@">>>CBCentralManagerStatePoweredOn");
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    ...
    if (error)
    {
        RBQLog4(@">>> didDisconnectPeripheral for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverServices:(NSError *)error {
    ...
    if (error) {
        RBQLog4(@">>>didDiscoverServices for %@ with error: %@", peripheral.name, [error localizedDescription]);
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error {
    ...
    if (error) {
        RBQLog4(@"error didDiscoverCharacteristicsForService for %@ with error: %@", service.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (error) {
        RBQLog4(@"error didUpdateValueForCharacteristic %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        return;
    }
    //查找字段订阅
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverDescriptorsForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if (error) {
        RBQLog4(@"error Discovered DescriptorsForCharacteristic for %@ with error: %@", characteristic.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    ...
    if (error) {
        RBQLog4(@"error didUpdateValueForDescriptor  for %@ with error: %@", descriptor.UUID, [error localizedDescription]);
        //        return;
    }
    //回叫block
...
//    RBQLog4(@">>> didWriteValueForCharacteristic uuid:%@",characteristic.UUID);
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForDescriptor:(CBDescriptor *)descriptor error:(NSError *)error {
    ...
    RBQLog4(@">>> didWriteValueForDescriptor uuid:%@",descriptor.UUID);
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    RBQLog4(@">>>didUpdateNotificationStateForCharacteristic");
    RBQLog4(@">>>uuid:%@,isNotifying:%@",characteristic.UUID,characteristic.isNotifying?@"isNotifying":@"Notifying");
...
- (void)peripheral:(CBPeripheral *)peripheral didReadRSSI:(NSNumber *)RSSI error:(NSError *)error {
    ...
    RBQLog4(@">>>peripheralDidUpdateRSSI -> RSSI:%@",RSSI);
    if ([currChannel blockOnDidReadRSSI]) {

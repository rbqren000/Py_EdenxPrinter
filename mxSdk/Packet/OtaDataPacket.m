...
/**
 *
 * @param data 数据  使用默认的帧头 STX_E  0x1E
 */
-(void)set:(NSData *)data {
    [self set:data fh:STX_E];
}
...
-(void)set:(NSData *)data fh:(int)fh {
    ...
    RBQLog3(@"ota数据 长度:%f k; 共分:%lu 包; usefulPacketDataLength:%ld",self.totalDataLen /1000.0f,(unsigned long)self.totalPacketCount,self.usefulPacketDataLength);
}
...
-(NSData *)getCurrentPacket {
    ...
    @autoreleasepool {
        ...
        if (offset + self.usefulPacketDataLength <= self.totalDataLen) {
            ...
            memset([lastPacket mutableBytes] + overPackSize, 0, self.usefulPacketDataLength - overPackSize);
            return lastPacket;
        }
    }
}
